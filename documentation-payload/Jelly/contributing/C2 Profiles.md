...
- ISerializer serializer - This object is used to prepare C# structures into a serialized format that Mythic will receive, and allow the profile to deserialize JSON messages from Mythic into Jelly structures. Currently this variable should not be modified in the agent core.
- IAgent agent - Core Jelly agent interface that grants the C2 profile access to other parts of the agent, such as the task manager.
...
public interface IC2Profile
{
    ...
    // where Jelly will receive the response. Used if a one way profile.
...
## Adding Your Profile to Jelly Core

Once you've created your new C2 profile, you'll need to add it to Jelly as a build option for C2 profiles.

In the Jelly project under the Jelly solution, add your new C2 profile as a project reference. Then, at the top of the `Jelly/Config.cs` file, add the following lines:
...
Lastly, you'll need to modify the builder.py file under `Payload_Type/Jelly/mythic/agent_functions`. In that file, add your new profile to the `c2profiles` attribute under Jelly.