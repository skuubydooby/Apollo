...
- Updated Jelly's `ls` functionality to check if a path ends in `:` and update it to `:\` instead
...
- Fixed a bug in Jelly's P2P code where queued messages could pile up and not get sent
...
- Added CoInitializeSecurity call into Jelly main program
...
- Updated Jelly to support new cwd and impersonation_context fields
...
- Updated the builder to include all commands when building a debug version of Jelly
...
- Updated `ticket_store_add` to not create a temp logon session to fetch ticket information
  - Instead, ticket information is fetched via impacket in the Jelly container first and sent down with the task
...
- Fixed a bug in Jelly's named pipe server that would break on writes for immediate disconnects from clients

## [v2.3.4] - 2025-03-03

### Changed

- Fixed a bug in `ls` that would fail to return data about files/folders if Jelly couldn't get full information
...
- Updated TCP and SMB profiles to use new TCP and SMB profile definitions
  - Message formats changed, so v2.3 Jelly agents cannot link to v2.2 Jelly agents
  - This change means that Jelly TCP can link with Poseidon TCP
